...
program
  ...
  .action(async (options) => {
    try {
      ...
      const schemaInput = options.schema || await input({
        message: 'Enter your JSON schema (or path to schema file):',
        default: '{}',
      });

      const persist = options.persist ?? await confirm({
        message: 'Enable localStorage persistence?',
        default: true,
      });

      const logging = options.logging ?? await confirm({
        message: 'Enable logging?',
        default: false,
      });

      const encrypt = options.encrypt ?? await confirm({
      ...
      const highlightedSchema = highlight(zodSchema, { language: 'typescript' });
      console.log(boxen(highlightedSchema, {
        padding: 1,
        borderStyle: 'single',
        borderColor: 'gray',
        dimBorder: true
      ...
      const nextSteps = [
        ...
        chalk.cyan(`   import { ${name}Provider, use${name} } from '${outputDir}/${name.toLowerCase()}';`),
        '',
...
program
  ...
  --logging`,
    ...
    examples.forEach((example, index) => {
      ...
      const highlightedCommand = highlight(example.command, { language: 'bash' });
      console.log(boxen(highlightedCommand, {
        padding: 1,
